...
import {
  ...
  Wallet, X, AlertTriangle, BarChart3, LayoutDashboard, Eye, EyeOff
} from 'lucide-react';
      ...
      <main className="relative z-10 flex-1 w-full p-6 grid grid-cols-12 gap-6 overflow-hidden max-w-[1700px]">
        <aside className="col-span-3 flex flex-col gap-6 overflow-y-auto pr-2 scrollbar-hide py-2 animate-in fade-in slide-in-from-left-4 duration-700">
          <GlassSection title="Patrimoine" icon={<Home size={18} className="text-indigo-400" />}>
            ...
            <PremiumInput label="Frais Notaire" value={activeSim.data.fraisNotaire} onChange={(v) => updateData('fraisNotaire', v)} />
          ...
          <GlassSection title="Revenus & Risques" icon={<AlertTriangle size={18} className="text-rose-400" />}>
            <PremiumInput label="Vacance Locative %" value={activeSim.data.vacanceLocative} onChange={(v) => updateData('vacanceLocative', v)} />
        ...
        <div className="col-span-9 flex flex-col gap-6 animate-in fade-in slide-in-from-right-4 duration-700">
          <div className="grid grid-cols-4 gap-6 h-36 shrink-0 pt-2">
            <HeroKPI label="Cashflow Réel" value={formatE(calculations.cashflowM)} color={calculations.cashflowM >= 0 ? "emerald" : "rose"} icon={<ArrowRightLeft />} highlight sub={`Net / mois (Vacance incl.)`} />
            <HeroKPI label="Rendement Net" value={`${calculations.rNet.toFixed(2)}%`} color="indigo" icon={<TrendingUp />} />
            <HeroKPI label="Gains (20 ans)" value={formatE(calculations.beneficeAn * 20)} color="emerald" icon={<Sparkles />} />
            <HeroKPI label="Enrichissement" value={formatE(calculations.beneficeAn * 20 + (calculations.investTotal - activeSim.data.apport))} color="indigo" icon={<Building2 />} sub="Total Net" />
          </div>
          ...
          <div className="flex-1 grid grid-cols-12 gap-6 min-h-0 bg-slate-900/40 backdrop-blur-xl border border-white/5 rounded-[2.5rem] p-8 shadow-2xl relative overflow-hidden">
            <div className="col-span-12 flex flex-col h-full">
              <div className="flex justify-between items-start mb-8 shrink-0">
                <div><h4 className="text-[10px] font-black uppercase text-indigo-400 tracking-[0.4em] flex items-center gap-2 mb-1"><BarChart3 size={14} /> Simulation 20 ans</h4><p className="text-xl font-bold text-white tracking-tight">Analyse dynamique du projet</p></div>
      ...
      <div className="fixed left-[-9999px] top-0 pointer-events-none">
        <div ref={reportRef} className="w-[210mm] p-[15mm] bg-white text-slate-900 flex flex-col gap-6 font-sans">
          ...
          <div className="grid grid-cols-4 gap-4">
            <RepKPI label="Rendement Net" value={`${calculations.rNet.toFixed(2)}%`} /><RepKPI label="Cashflow Net" value={formatE(calculations.cashflowM)} highlight /><RepKPI label="Mensualité" value={formatE(calculations.mCredit)} /><RepKPI label="Gains (20 ans)" value={formatE(calculations.beneficeAn * 20)} />
          </div>
          <div className="grid grid-cols-2 gap-10">
            <section className="bg-slate-50 p-6 rounded-[2rem] border border-slate-100"><h3 className="text-xs font-black uppercase text-slate-900 border-b-2 border-indigo-500 mb-4 pb-1">Financement</h3><div className="space-y-2 text-sm"><RepRow label="Prix d'achat" value={formatE(activeSim.data.prixAchat)} /><RepRow label="Travaux" value={formatE(activeSim.data.travaux)} /><div className="flex justify-between font-black text-lg border-t-2 pt-2 border-slate-900 mt-2"><span>INVESTISSEMENT</span><span>{formatE(calculations.investTotal)}</span></div><RepRow label="Apport Personnel" value={formatE(activeSim.data.apport)} /><RepRow label="CRÉDIT BANCAIRE" value={formatE(calculations.loanAmount)} /></div></section>
            <section className="bg-slate-50 p-6 rounded-[2rem] border border-slate-100"><h3 className="text-xs font-black uppercase text-slate-900 border-b-2 border-indigo-500 mb-4 pb-1">Analyse Mensuelle</h3><div className="space-y-2 text-sm"><RepRow label="Loyers (Vacance incl.)" value={formatE(calculations.recetteMensuelleRéelle)} /><RepRow label="Échéance Crédit" value={formatE(calculations.mCredit)} /><RepRow label="Charges / mois" value={formatE(calculations.totalChargesAnnuelles / 12)} /><div className="flex justify-between font-black text-lg border-t-2 pt-2 border-emerald-500 mt-2 text-emerald-600"><span>CASHFLOW NET</span><span>{formatE(calculations.cashflowM)}</span></div></div></section>
          </div>
          <div><h3 className="text-[10px] font-black uppercase text-slate-400 border-b mb-3 pb-1">Détails de l'exploitation (par an)</h3><div className="grid grid-cols-2 gap-x-12 gap-y-1.5">{activeSim.data.charges.map(c => <div key={c.id} className="flex justify-between text-[11px] border-b pb-1"><span className="text-slate-500 font-bold uppercase">{c.name}</span><span className="font-black text-slate-900">{formatE(c.value)}</span></div>)}</div></div>
          <div className="mt-auto p-10 bg-indigo-600 rounded-[3rem] text-white flex justify-between items-center shadow-2xl shadow-indigo-100"><div><h3 className="text-2xl font-black mb-2 uppercase tracking-tighter italic">Opportunité Validée</h3><p className="text-indigo-100 text-sm">Le projet présente un enrichissement net de <span className="font-black underline text-white">{formatE(calculations.beneficeAn * 20 + (calculations.investTotal - activeSim.data.apport))}</span> sur 20 ans.</p></div><div className="bg-white p-8 rounded-[2.5rem] text-indigo-900 shadow-xl text-center"><p className="text-[10px] uppercase font-black opacity-60 mb-1">Total Net à terme</p><p className="text-4xl font-black tracking-tighter">{formatE(calculations.beneficeAn * 20 + (calculations.investTotal - activeSim.data.apport))}</p></div></div>
        </div>
      </div>
    </div>
  );
...
function DimensionToggle({ active, onClick, dot, label }) {
  return (
    <button onClick={onClick} className={`px-4 py-2 rounded-full border flex items-center gap-2 transition-all ${active ? 'bg-white/10 border-indigo-500 text-white' : 'border-white/5 text-slate-500 opacity-50 hover:opacity-100 hover:bg-white/5'}`}>
