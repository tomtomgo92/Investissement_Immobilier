...
// ⚡ Bolt Optimization: Extracted DealCard and wrapped in React.memo to prevent
// O(N) recalculations of pipeline metrics when unrelated cards or state change.
const DealCard = React.memo(({ sim, colIndex, toggleAlert, openSimulation, changeStatus }) => {
  ...
  return (
    <div className="bg-white dark:bg-slate-900 p-3 rounded-lg border border-slate-200 dark:border-slate-700 shadow-sm hover:shadow-md transition-shadow group">
      <div className="flex justify-between items-start mb-2">
        ...
        <button
          onClick={() => toggleAlert(sim.id, sim.name, sim.hasAlert)}
...
export default function DealPipeline({ simulations, setSimulations, setActiveSimId, setViewMode }) {
  ...
  const toggleAlert = useCallback((simId, simName, hasAlert) => {
    setSimulations(prev => prev.map(s => {
      if (s.id === simId) {
        const isEnabling = !hasAlert;
        if (isEnabling) {
