...
// Optimize card rendering to prevent O(N) recalculations on unrelated state updates
// by isolating computationally expensive metrics calls within individual React.memo components
const PipelineCard = React.memo(({ sim, colIndex, toggleAlert, openSimulation, changeStatus }) => {
  // Localize expensive metrics calculation to avoid running for all items on parent render
  ...
  return (
    <div className="bg-white dark:bg-slate-900 p-3 rounded-lg border border-slate-200 dark:border-slate-700 shadow-sm hover:shadow-md transition-shadow group">
      <div className="flex justify-between items-start mb-2">
        ...
        <button
          onClick={() => toggleAlert(sim.id, sim.name, hasAlert)}
...
export default function DealPipeline({ simulations, setSimulations, setActiveSimId, setViewMode }) {
  const toggleAlert = useCallback((simId, name, currentAlertStatus) => {
    setSimulations(prev => prev.map(s => {
      if (s.id === simId) {
        const isEnabling = !currentAlertStatus;
        if (isEnabling) {
          alert(`Alerte activée pour "${name}". Vous serez notifié si le prix baisse ou dans 45 jours.`);
        }
    ...
    <div className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 p-6 shadow-sm animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="flex gap-4 overflow-x-auto pb-4 items-stretch min-h-[500px]">
        {COLUMNS.map((col, colIndex) => {
          // Avoid mapping all elements before filtering, instead filter the original list directly
