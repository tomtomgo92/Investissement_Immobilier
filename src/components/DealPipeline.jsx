...
// ⚡ Bolt Optimization: Extracted simulation card into a memoized component.
// Impact: Prevents O(N) recalculations of pipeline metrics for all cards
// when a single card's status or alert is toggled.
const PipelineCard = memo(({ sim, colIndex, openSimulation, toggleAlert, changeStatus }) => {
  const metrics = useMemo(() => calculatePipelineMetrics(sim.data), [sim.data]);
    ...
    <div className="bg-white dark:bg-slate-900 p-3 rounded-lg border border-slate-200 dark:border-slate-700 shadow-sm hover:shadow-md transition-shadow group">
      ...
      <div className="space-y-1 mb-3">
        <div className="flex items-center justify-between text-xs">
          ...
          <span className="font-semibold text-slate-700 dark:text-slate-300">{formatE(metrics.investTotal)}</span>
        </div>
        <div className="flex items-center justify-between text-xs">
          <span className="flex items-center gap-1 text-slate-500"><Wallet size={10} /> Cashflow</span>
          <span className={`font-bold ${metrics.cashflowNetNet >= 0 ? 'text-emerald-500' : 'text-rose-500'}`}>
            {formatE(metrics.cashflowNetNet)}/m
          </span>
        </div>
        <div className="flex items-center justify-between text-xs">
          <span className="flex items-center gap-1 text-slate-500"><TrendingUp size={10} /> Renda. Net</span>
          <span className="font-bold text-indigo-500">{metrics.rNet.toFixed(2)}%</span>
        </div>
      </div>
