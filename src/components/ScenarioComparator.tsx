...
export default function ScenarioComparator({ simulations, activeSimId, setActiveSimId }: ScenarioComparatorProps) {
  ...
  return (
    <div className="bg-white/60 dark:bg-slate-900/40 backdrop-blur-xl rounded-2xl border border-white/50 dark:border-slate-800/50 overflow-hidden shadow-[0_2px_10px_rgba(0,0,0,0.02)] animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-left border-collapse">
          <thead>
            <tr>
              <th className="p-6 w-48 bg-slate-50/80 dark:bg-slate-800/50 border-b border-slate-200/60 dark:border-slate-800 text-[10px] font-bold uppercase text-slate-400 tracking-[0.2em] sticky left-0 z-10 backdrop-blur-sm">
                Métrique Clé
              </th>
              {comparedData.map(sim => (
                <th key={sim.id} className={`p-6 min-w-[220px] border-b border-slate-200/60 dark:border-slate-800 transition-colors ${activeSimId === sim.id ? 'bg-indigo-50/50 dark:bg-indigo-900/10' : ''}`}>
          ...
          <tbody className="text-sm">
            ...
            {/* Investment Section */}
            <tr className="bg-slate-50/50 dark:bg-slate-800/30"><td colSpan={simulations.length + 1} className="p-3 px-6 text-[10px] font-bold text-slate-400 uppercase tracking-[0.2em]">Investissement</td></tr>

            <tr className="hover:bg-slate-50/30 dark:hover:bg-slate-800/20 transition-colors">
              <td className="p-6 font-semibold text-slate-500 text-xs tracking-wide sticky left-0 bg-white/60 dark:bg-slate-900/40 backdrop-blur-sm">Prix Total</td>
              {comparedData.map(sim => (
                <td key={sim.id} className="p-6 border-b border-slate-100 dark:border-slate-800/50 font-bold text-slate-800 dark:text-slate-200">
                  {formatE(sim.investTotal)}
                </td>
              ))}
            </tr>
            <tr className="hover:bg-slate-50/30 dark:hover:bg-slate-800/20 transition-colors">
              <td className="p-6 font-semibold text-slate-500 text-xs tracking-wide sticky left-0 bg-white/60 dark:bg-slate-900/40 backdrop-blur-sm">Apport</td>
              {comparedData.map(sim => (
                <td key={sim.id} className="p-6 border-b border-slate-100 dark:border-slate-800/50 font-bold text-slate-800 dark:text-slate-200">
             ...
             {/* Performance Section */}
             <tr className="bg-slate-50/50 dark:bg-slate-800/30"><td colSpan={simulations.length + 1} className="p-3 px-6 text-[10px] font-bold text-slate-400 uppercase tracking-[0.2em]">Rentabilité & Cashflow</td></tr>

            <tr className="hover:bg-slate-50/30 dark:hover:bg-slate-800/20 transition-colors">
              <td className="p-6 font-semibold text-slate-500 text-xs tracking-wide flex items-center gap-3 sticky left-0 bg-white/60 dark:bg-slate-900/40 backdrop-blur-sm">
              ...
              {comparedData.map(sim => {
                ...
                return (
                  <td key={sim.id} className={`p-6 border-b border-slate-100 dark:border-slate-800/50 font-bold ${sim.cashflowNetNet >= 0 ? 'text-emerald-600 dark:text-emerald-400' : 'text-rose-600 dark:text-rose-400'} ${isBest ? 'bg-emerald-50/50 dark:bg-emerald-900/20' : ''}`}>
            ...
            <tr className="hover:bg-slate-50/30 dark:hover:bg-slate-800/20 transition-colors">
              ...
              {comparedData.map(sim => {
                ...
                return (
                  <td key={sim.id} className={`p-6 border-b border-slate-100 dark:border-slate-800/50 font-bold text-slate-800 dark:text-slate-200 ${isBest ? 'bg-indigo-50/50 dark:bg-indigo-900/20' : ''}`}>
                    {sim.rNet.toFixed(2)}%
                  </td>
                );
              })}
            ...
            <tr className="hover:bg-slate-50/30 dark:hover:bg-slate-800/20 transition-colors">
              <td className="p-6 font-semibold text-slate-500 text-xs tracking-wide sticky left-0 bg-white/60 dark:bg-slate-900/40 backdrop-blur-sm">Fiscalité Optimale</td>
              {comparedData.map(sim => (
                <td key={sim.id} className="p-6 border-b border-slate-100 dark:border-slate-800/50 text-xs">
            ...
            {/* Projection Section */}
            <tr className="bg-slate-50/50 dark:bg-slate-800/30"><td colSpan={simulations.length + 1} className="p-3 px-6 text-[10px] font-bold text-slate-400 uppercase tracking-[0.2em]">Projection (20 Ans)</td></tr>
