...
export default function BankabilityIndicator({ bankability }: BankabilityIndicatorProps) {
  ...
  const getStatusColor = () => {
    switch (status) {
      ...
      default: return 'text-slate-500 bg-slate-50 border-slate-200 dark:bg-white/[0.05] dark:border-white/[0.08]';
    }
  };
  ...
  return (
    <div className={cn(
      ...
      <div className="flex items-center gap-3.5">
        <div className="p-2.5 bg-white/60 dark:bg-[#0B0F19]/60 backdrop-blur-sm rounded-xl shadow-sm border border-white/20 dark:border-white/[0.05]">
        ...
        <div>
          ...
          <div className="flex items-baseline gap-2">
            ...
            <span className="text-[10px] font-medium tracking-wide uppercase opacity-90 px-2 py-0.5 rounded-full bg-white/40 dark:bg-white/[0.02]">
              {getStatusText()}
            </span>
          </div>
        </div>
