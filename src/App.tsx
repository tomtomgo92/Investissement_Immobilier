  ...
  const confirmImport = () => {
    ...
    setSimulations(p => p.map(s => {
      ...
      const loyerMensuelTotal = s.data.typeLocation === 'courte_duree'
           ...
           : getLoyersArr(s.data.loyers).reduce((acc: number, val: number) => acc + val, 0);
          ...
          <>
                            ...
                            <div className="relative flex-1">
                              <input
                                ...
                                onChange={(e) => {
                                  ...
                                  setSimulations(p => p.map(s => {
                                    if (s.id !== activeSimId) return s;
                                    const nl = [...getLoyersArr(s.data.loyers)];
                                    nl[i] = v;
            ...
            <ReverseCalculator
              ...
              onApplyMinRent={(rent) => {
                 setSimulations(p => p.map(s => {
                    if (s.id !== activeSimId) return s;
                    return { ...s, data: { ...s.data, loyers: getLoyersArr(s.data.loyers).map(() => rent) } };
                 }));
              }}
            />
